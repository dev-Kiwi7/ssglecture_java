...
import com.sinse.shop.product.view.ProductCard;
...
public class MainPage extends Page{
	...
	public MainPage(AppMain appMain) {
		...
		p_content = new JPanel(new FlowLayout(FlowLayout.LEFT, 15, 0));
		productDAO = new ProductDAO();
		recentList = productDAO.selectRecent();
		
		//스타일
		p_visual.setPreferredSize(new Dimension(Config.MAIN_VISUAL_WIDTH, Config.MAIN_VISUAL_HEIGHT));
		p_content.setPreferredSize(new Dimension(Config.SHOPMAIN_WIDTH-50, 410));
	...
	public void createProduct() {
		for(int i=0;i<recentList.size();i++) {
			Product product=recentList.get(i);
			ProductCard productCard=new ProductCard(product);
			p_content.add(productCard);
		}
